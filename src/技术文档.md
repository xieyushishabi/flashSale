...
|------|----------|----------|-------------|
| Redis | 内存Map模拟 | Redis集群 | 使用Redis集群 |
| Elasticsearch | 数组过滤 | ES集群 | 使用ES集群 |
| RabbitMQ | 内存队列 | RabbitMQ集群 | 使用消息队列集群 |
| WebSocket | 简单广播 | Socket.IO | 使用成熟的WebSocket方案 |
| MongoDB | ✅ 真实数据库 | ✅ 真实数据库 | 继续使用 |
| JWT | ✅ 自实现 | 第三方库 | 可升级为成熟方案 |
